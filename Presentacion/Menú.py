...
class Menu:
    ...
    def menu(self):
        Label(self.root, text="Elige una opción:", font=("Helvetica", 14)).pack(
            pady=10
        )  # Fuente y espacio alrededor
        ...
        Button(
            ...
            text="Crear entrada",
            width=15,
            command=self.opcion_crear_entrada,
        ).grid(row=0, column=0, padx=5)
        Button(
            options_frame,
            text="Ver entradas",
            width=15,
            command=self.opcion_ver_entradas,
        ).grid(row=0, column=1, padx=5)
        Button(
            options_frame,
            text="Ver entrada específica",
            width=15,
            command=self.opcion_ver_entrada_especifica,
        ).grid(row=1, column=0, padx=5)
        Button(
            options_frame,
            text="Actualizar entrada",
            width=15,
            command=self.opcion_actualizar_entrada,
        ).grid(row=1, column=1, padx=5)
        Button(
            options_frame,
            text="Eliminar entrada",
            width=15,
            command=self.opcion_eliminar_entrada,
        ).grid(row=2, column=0, columnspan=2, pady=5)

        # Botón de salir alineado a la derecha
        Button(self.root, text="Salir", width=10, command=self.opcion_salir).pack(
    ...
    def opcion_crear_entrada(self):
        ...
        contenido = simpledialog.askstring(
            "Input", "Introduce el contenido de la entrada: "
        )
    ...
    def opcion_ver_entrada_especifica(self):
        id_entrada = simpledialog.askstring(
            "Input", "Introduce el ID de la entrada a leer: "
        )
        ...
        if entrada:
            ...
            messagebox.showinfo(
                "Mensaje", "No se encontró la entrada con el ID especificado."
            )

    def opcion_actualizar_entrada(self):
        id_entrada = simpledialog.askstring(
            "Input", "Introduce el ID de la entrada a actualizar: "
        )
        titulo = simpledialog.askstring(
            "Input", "Introduce el nuevo título de la entrada: "
        )
        contenido = simpledialog.askstring(
            "Input", "Introduce el nuevo contenido de la entrada: "
        )
    ...
    def opcion_eliminar_entrada(self):
        id_entrada = simpledialog.askstring(
            "Input", "Introduce el ID de la entrada a eliminar: "
        )
